- problems like volatility ('slight input changes causing volatility in output') can be fixed with some general structures of optimization, like generalization (rather than assigning a specific position, assign an approximate or general position involving the original node & neighboring nodes based on similarity, as in nodes with similar outputs), or approximation structures based on the insight that 'a very specific answer (in the form of a prediction function) created by non-specific network params is unlikely to be optimal, bc the level of certainty achievable with a network is not create-able with the input certainty, and it cant come from nowhere'
