...
- if you know information like the following, other information will be useful to calculate that
    ...
    - asking questions like the following are useful in finding these useful problems to solve and useful interface queries to solve them
        ...
        - what problems can be solved (what attributes detected/verified/filtered, what other structures found) with what structures (attributes, sequence patterns, proved limits/connections)
        - what structures (extremes, opposites, components, comparison type-enabling standards like bases) are the most useful for the most useful problems to solve (like 'finding the limit on the possible functionality of a input type/function set')
            - similarly what interface structures (like a base, self, fractal, example, representation) are useful for other common intents (like 'describe the structure optimally' or 'find components of these structures & composable structures (which can be built with these structures)') applied to these structures
        - what is the reason that a connection between structures develops/holds (why does it become a common useful structure such as a 'symmetry') rather than degrades?
            - for example, why do linear combinations of linear combinations preserve data type of input? 
                - bc of the uniformity and 'constant' change type of the change being applied, rather than being differently applied across terms of the function, and rather than being a variable change type
                - without additional change types (such as function range limits or variable multipliers or variable exponents) other than constant multipliers, other change types cant be created
    ...
    - what other intents are of interest, other than 'automating proofs', or 'extracting information about an input data type and its possible uses/attributes with a particular function'?
        ...
        - finding useful structures to solve for and apply as constants/maps (such as how rainbow tables are useful to solve for, similarly a useful graph of 'overlapping structures of variation created by various operations' is useful to solve for)
            - finding useful structures like 'maps of provability' where attribute sets are graphed in a way that depicts provability of various problems (problems formatted in a standard structure such as 'points to connect' or 'differences to resolve'), such as attribute sets/sequences ('addition' and 'input' attributes) that allow for proving 'preservation of input data type in addition of linear functions'
                - a specific map might indicate the 'connections that can be proven using a specific structure like an attribute (such as associativity)', and a general map might indicate 'attribute set-problems solvable' map or a set of specific attribute proof maps to indicate overlaps in provable connections
