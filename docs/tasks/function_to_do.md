...
## examples
  ...
  - add to math
    ...
    - given that the function of 'rotation' can create an 'opposite' structure which is useful for connecting/creating differences, can this be extended to create all useful difference types (or all useful opposites), meaning "can a 'structure' be rotated to create 'nothing'", and why is the rotation the cause of this useful structural difference
      ...
      - what is the connection between opposite-creating structures like 'rotation' and output difference structures like 'reverse' - does one tend to hide information of differences (creating effects such as 'blur' which create the illusion of solidity), and the other tends to hide different information (like 'state sequence or trajectory to reach a point' and the 'starting point')
        ...
        - what shapes gain/create information when rotated (creating new differences with each rotation, like 'infinite sets in an infinite set' which are just different enough (a different infinity, but still an infinity) to be isolatable/measurable), as opposed to shapes that lose information when rotated (shapes with uniformities like circles where differences can be embedded while losing the representation)
      - the corresponding question in physics is 'how does this relate to cpt symmetry'
    - the alignment of structures like 'cause', 'intent', 'example input/output sets', and 'function (change sequence/network)' is in their common structure of 'direction' which makes them similarly useful, adjacent, and possible alternative information sources
