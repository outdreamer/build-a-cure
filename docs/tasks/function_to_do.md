...
## examples
  ...
  - add to ml
    ...
    - add to structures
      ...
      - what functions arent adjacent to the existing functions in the logic? (beyond biological agent functions like 'trade', 'fool', 'simulate', 'teach')
      ...
      - useful variable structures
        ...
        - useful variables are 'position' and 'count' bc most variables can be standardized to these variables ('removing insignificant repetitions' as a useful problem-solving intent) to identify significant differences, then re-apply differences to get to a useful interaction level and re-apply position/count in that more useful interaction level
        ...
        - finding the maximum error structures (like extreme over-prioritizations) to avoid is useful to find 'limits' and 'sequences leading to limits' (like 'painting into a corner' being an error of not recognizing 'irreversible steps' that should be done last in a sequence, otherwise a 'dead end' error is reached requiring the painter to stop moving), to identify the 'starting points of these sequences leading to these limits' as useful to identify and trivial to identify once the over-prioritization and other error structures of a problem space are known (applying common error structures like 'irreversibilities' of a space)
